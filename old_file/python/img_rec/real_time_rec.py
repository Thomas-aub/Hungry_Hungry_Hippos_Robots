...
def scale_points(points):
    # return [[x*2 for x in point] for point in points]
    # return np.array(points * 2).astype(np.float32)
    return points * 2
...
def processing(frame):
    ...
    # old south one from entry of room
    # new west
    ...
    # corres_finder("TOP LEFT", top_left, arena)
    ...
    try:
        ...
        while client.connected:
            if client.frame is not None:
                ...
                frame2 = cv2.resize(client.frame, (1280, 720))
                # cv2.imshow("test_resized", frame2)
