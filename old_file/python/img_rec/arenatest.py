...
def corres_finder(src, position):
	cv2.imshow("src", src)
	cv2.setMouseCallback("src", click_event, param="src")
	print("Distance : ", sqrt(position[0] - lastPoint[0][0])**2 + (posY - lastPoint[0][1])**2)
	cv2.waitKey(0)
	cv2.destroyAllWindows()
	
arena = cv2.imread("./arena.png")
posX = 256
posY = 256
cv2.circle(arena, (posX, posY), 5, (255, 255, 255))
corres_finder(arena, (posX, posY))
#def toScreenPoint(c):
...
#    cv2.circle(screen,bodyColor,pos.tuple(),robot._size/2.0)
#    cv2.circle(screen,frontColor,pos.tuple(),tool.tuple(),1)
#    cv2.circle(screen,wheelColor,left.tuple(),3.0)
#    cv2.circle(screen,wheelColor,right.tuple(),3.0)
